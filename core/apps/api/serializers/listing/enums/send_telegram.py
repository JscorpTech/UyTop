...
def send_telegram(listing):
    ...
    for admin_id in settings.ADMIN:
        if media_group:
            bot.send_media_group(admin_id, media=media_group)
        else:
            bot.send_message(admin_id, captio_text(listing, map_url), parse_mode="HTML")
...
def send_check(check):
    ...
    button.add(
        types.InlineKeyboardButton(
            text="Tasdiqlash ✅",
            callback_data=f"check_{check.listing.id}_{check.listing.user.tg_id}"
        )
    )
